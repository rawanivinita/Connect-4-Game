...
public class TextView implements IView
{
	...
	public GameSettings requestGameSettings()
	{
		...
		while (nrRows< IModel.MIN_ROWS || nrRows>IModel.MAX_ROWS) {
			System.out.println("Error, enter a number between 3 and 10");
		...
		int min = Math.min(nrRows,nrCols);

		System.out.println("Enter the number for Streak");
		int streak = InputUtil.readIntFromUser();
		while (streak < 3 || streak > min){
			System.out.println("Error, enter 3 or the maximum " + min + " or between those numbers");
